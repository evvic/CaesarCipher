...
int main()
{
    ...
    //Declare strings for outName and outPrefix
    ...
    //Prompt and read output file prefix
    printf("Output file prefix: ");
    scanf("%s", outPrefix);
    printf("\n");

    //Copy prefix to output file string, concatenate input file name
    strcat(outPrefix, inName);

    //Open output file.  If it fails, exit the program.
    ...
    //Prompt and read the key.  Don't forget the & for the int on scanf!!
    printf("Key: ");
    scanf("%d", &key);
    printf("\n");
...
///validFileSuffix - check file is a .txt
int validFileSuffix(char str[]) {
    int lel = strlen(str);
